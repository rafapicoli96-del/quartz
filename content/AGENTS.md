...
If file already exists, append with a `---` divider. Otherwise create it.
...
GCal rules: Show "#️⃣ 🏋️ 🥗 😑" EXACTLY as in GCal — NEVER rename. Skip all-day ⏰ events. Mixed blocks: show as written, do NOT infer individual durations within a block.
...
[não preenchido — Rafa preenche ao fechar o dia]
...
[Skip all-day ⏰ reminders. Show "#️⃣ 🏋️ 🥗 😑" exactly as-is. Mixed blocks: show as written.]
...
Read `Today.md` and `Work — Tarefas Ativas.md`. Extract actionable items from Today.md not already in Work file. Assign priority (🔴/🟡/🟢) and category. Update `Master To-Do.md`: ADD new items, REMOVE completed, KEEP unchecked.

```bash
git add 'Master To-Do.md'
